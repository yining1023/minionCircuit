...
function setup() {
  ...
  frownImg = loadImage("imgs/minionFrown.png");
  minionImg = loadImage("imgs/croppedMinion.png");
  closedCircuitImg = loadImage("imgs/closedCircuit.png");
  openCircuitImg = loadImage("imgs/openCircuit.png");
}//END OF SETUP
...
// }
	...
	socket.on('sensor', function(data) {
		...
		if(data == 1){
			...
			image(minionImg,-50,0);
			//THIS IS THE NORMAL HEART
			image(closedCircuitImg, 715, 355, closedCircuitImg.width/10, closedCircuitImg.height/10);
		}
		else if(data == 0){
			// console.log("circuit is broken.");
			image(frownImg,195,400);
			//THIS IS THE BROKEN HEART
			image(openCircuitImg, 715, 355, openCircuitImg.width/10, openCircuitImg.height/10);
		}
